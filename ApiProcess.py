...
def get_presence(from_date, to_date):
    ...
    response = presenceSession.get(presence_host + "api/presence/v1/dwell/count?siteId=" + siteId + "&startDate=" +
...
def get_repeat_visitors(from_date, to_date):
    ...
    response = presenceSession.get(presence_host +
                                   "api/presence/v1/repeatvisitors/average?siteId=" + siteId + "&startDate=" +
                                   from_date + "&endDate=" + to_date)
    return response.json()
